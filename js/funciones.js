...
function guardarTablero(nombre){
	...
	tableros.innerHTML+="<div class='tablero'>"+"<i class='fa fa-thumb-tack' aria-hidden='true'></i>"+
	"<h1>"+nombre+"</h1>"+"</div>";
...
function guardarLugar(){
	...
	//ENVIAR CON AJAX
	enviar = new XMLHttpRequest(); //CREAMOS LA INSTANCIA DEL OBJETO
	// cadena de variables
	variables = 'nombre='+nombre.value+'&precio='+precio.value+'&descripcion='+descripcion.value+'&categoria='+
	categoria.value+'&latitud='+latitud+'&longitud='+longitud+'&idTablero='+idTablero;
	enviar.open('GET','php/guarda.php?'+variables);
	enviar.send();
	enviar.onreadystatechange=function(){
		//CUANDO YA HAYA RESPUESTA
		if (enviar.status == 404) {
